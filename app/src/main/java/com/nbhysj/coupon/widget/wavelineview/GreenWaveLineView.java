...
public class GreenWaveLineView extends RenderView {
    ...
    protected void onRender(Canvas canvas, long millisPassed) {
        ...
        if (lineAnim(canvas)) {
            ...
            for (int i = 0; i < paths.size(); i++) {
                paths.get(i).moveTo(width, centerHeight/2);
            }
    ...
    private boolean lineAnim(Canvas canvas) {
        ...
        for (int i = 1; i <= samplingSize; i++) {
            ...
            paths.get(0).lineTo(x, 0);
            paths.get(1).lineTo(width - x, 0);

        }

        paths.get(0).moveTo(width/2, 0);
        paths.get(1).moveTo(width/2, 0);
