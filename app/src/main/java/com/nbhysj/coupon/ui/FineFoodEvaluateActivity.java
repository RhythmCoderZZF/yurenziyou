...
import com.nbhysj.coupon.model.response.MchFoodCommentBean;
...
/**
 * @auther：hysj created on 2019/12/01
 * description：美食评价
 */
public class FineFoodEvaluateActivity extends BaseActivity<FineFoodPresenter, FineFoodModel> implements FineFoodContract.View {
    //照片选取返回code
    ...
    public void initView(Bundle savedInstanceState) {
        ...
        GridLayoutManager gridLayoutManager = new GridLayoutManager(FineFoodEvaluateActivity.this, 3);
        gridLayoutManager.setOrientation(gridLayoutManager.VERTICAL);
        mRvOrderEvaluatePictureSelect.addItemDecoration(new RecyclerViewItemDecoration(Tools.dip2px(FineFoodEvaluateActivity.this, 10)));
        mRvOrderEvaluatePictureSelect.setLayoutManager(gridLayoutManager);

        orderEvaluateAdapter = new OrderCommentPictureAdapter(FineFoodEvaluateActivity.this, new OrderCommentPictureAdapter.OrderCommentPictureListener() {
            @Override
    ...
    public void getNotePhotoPicker(Set<MimeType> mimeTypes, boolean capture) {
        AndPermission.with(FineFoodEvaluateActivity.this)
                .permission(Manifest.permission.CAMERA, Manifest.permission.READ_EXTERNAL_STORAGE)
                .callback(new PermissionListener() {
                    @Override
                    public void onSucceed(int requestCode, @NonNull List<String> grantPermissions) {

                        Matisse.from(FineFoodEvaluateActivity.this)
...
//        OSSClient的生命周期和应用程序的生命周期保持一致即可。在应用程序启动时创建一个ossClient，在应用程序结束时销毁即可。
        ...
        OSSLog.enableLog();
        return new OssService(oss, bucket, displayer, FineFoodEvaluateActivity.this);
    ...
    public void getFoodCommentIndexResult(BackResult<FineFoodCommentInitResponse> res) {
        ...
        switch (res.getCode()) {
            case Constants.SUCCESS_CODE:
                try {
                    FineFoodCommentInitResponse orderGoodsInitResponse = res.getData();
                    String orderNo = orderGoodsInitResponse.getOrderNo();
                    String goodsType = orderGoodsInitResponse.getGoodsType();
                    List<MchFoodCommentBean> orderCommentMchList = orderGoodsInitResponse.getMch();
                    MchFoodCommentBean mchFoodCommentBean = orderCommentMchList.get(0);
                    String mchName = mchFoodCommentBean.getMchName();

                    //商户名称
                    if (!TextUtils.isEmpty(mchName)) {
                        mTvMchName.setText(mchName);
                    }
                    List<OrderCommentMchTagBean> commentMchTagList = mchFoodCommentBean.getMchTag();
            ...
            default:
                dismissProgressDialog();
                showToast(FineFoodEvaluateActivity.this, Constants.getResultMsg(res.getMsg()));
                break;
        }
    }
    ...
    public void showMsg(String msg) {
        ...
        dismissProgressDialog();
        showToast(FineFoodEvaluateActivity.this, Constants.getResultMsg(msg));
    }
    ...
    public void orderEvaluate() {
        ...
        if (validateInternet()) {
            ...
            if (TextUtils.isEmpty(orderCommentDes)) {
                ...
                showToast(FineFoodEvaluateActivity.this, "请输入评价描述");
                return;
            }
...
*/
            showProgressDialog(FineFoodEvaluateActivity.this);
    ...
    //单个订单评价初始化页面
    public void getFoodCommentIndexInit() {
        if (validateInternet()) {

            showProgressDialog(FineFoodEvaluateActivity.this);
            mPresenter.getFoodCommentIndex(mchId);
        }
    }
