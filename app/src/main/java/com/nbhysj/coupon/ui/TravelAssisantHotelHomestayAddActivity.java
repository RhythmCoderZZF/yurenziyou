...
/**
 * @auther：hysj created on 2019/07/10
 * description：添加酒店民宿
 */
public class TravelAssisantHotelHomestayAddActivity extends BaseActivity<TravelAssistantPresenter, TravelAssistantModel> implements TravelAssistantContract.View, TravelAssisantAddHotelHomestayFragment.AddHomestayListener {
                    ...
                    if (countyList != null) {
                        if (countyList.size() > 0) {
                            for (int i = 0; i < countyList.size(); i++) {
                                ...
                                fragments.add(new TravelAssisantAddHotelHomestayFragment().newInstance(mTripId, countyId, mDayIndex));
                            }
                            travelAssistantCountryPagerAdapter.setTravelAssistantCountryList(fragments, countyList);
    ...
    public void showMsg(String msg) {
        ...
        dismissProgressDialog();
        showToast(TravelAssisantHotelHomestayAddActivity.this, Constants.getResultMsg(msg));
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.rlyt_back:
                ...
                TravelAssisantHotelHomestayAddActivity.this.finish();

                break;
            case R.id.img_add_scenic_spot:

                Intent intent = new Intent();
                intent.setClass(TravelAssisantHotelHomestayAddActivity.this, TravelAssistanSelectCountyActivity.class);
